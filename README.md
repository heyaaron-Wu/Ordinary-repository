## 半自动人工智能系统开发综述 (Project Overview)
...
## 核心功能
* **响应式设计**：完美适配不同萤幕尺寸，并具备智慧隐藏式导览列（Auto-Hide Navigation）。
* **互动式体验**：
    * **影片弹窗 (Video Modal)**：点击后可即时播放产品介绍影片。
    * **平滑滚动**：优化的滚动逻辑，提升用户浏览流畅度。
...
## 核心机制
* **信号融合引擎**：整合政策、新闻与板块热度，生成具备权重的投资建议。
* **证据门控系统**：所有买入指令（BUY）必须通过数据完整性校验与幻觉防御，未经审计的决策将被自动拦截。
* **合规风控中心**：自动解析每只基金的 T+n 确认规则与惩罚费率，计算仓位敞口与止损点位。
...
本项目是一个由 AI 辅助设计与生成的项目。

生成方式： 全部的代码实现、文档结构及逻辑分析均通过 [Gemini与qwen] 与人类开发者的深度对话生成。

核心价值： 旨在展示 AI 在复杂量化金融交易系统中的代码生成能力与自动化集成潜力。
